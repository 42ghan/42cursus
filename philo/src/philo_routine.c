...
void	*monitor_death(void *arg)
{
	...
	{
		*(philo->v_flag) = 1;
		printf("\033[0;31m%ld\033[0m", time_cal(philo->start_t));
		printf("ms %d died\n", philo->idx);
	}
...
static void	philo_eat(t_philo *philo)
{
	...
	printf("\033[0;31m%ld\033[0mms %d has taken a fork\n",
		time_cal(philo->start_t), philo->idx);
	printf("\033[0;31m%ld\033[0mms %d has taken a fork\n",
		time_cal(philo->start_t), philo->idx);
	printf("\033[0;31m%ld\033[0mms %d is eating\n",
		time_cal(philo->start_t), philo->idx);
	(philo->n_eat)++;
...
void	*philo_action(void *arg)
{
	...
	{
		philo_eat(philo);
		printf("\033[0;31m%ld\033[0mms %d is sleeping\n",
			time_cal(philo->start_t), philo->idx);
		ft_usleep(philo->opts.time_slp * 1000);
		printf("\033[0;31m%ld\033[0mms %d is thinking\n",
			time_cal(philo->start_t), philo->idx);
	}
	return (NULL);
}
